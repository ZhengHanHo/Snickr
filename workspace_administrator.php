...
if ( !(isset( $_SESSION['username']) && isset($_GET['admin']) && $_SESSION['username'] == $_GET['admin']) ):    //用户尚未登录，必须重新登录
    ...
    $conn = custom_connect();
if (isset($_GET['uname'])): //当用户点击choose按钮时重新加载页面
    ...
    $still_member = is_still_member($invitedUserNameID, $workspaceID);
    $not_deleted_removed = workspaceDeleted_memberRemoved($adminID, $workspaceID);
    /************************************************************
    ...
    ?>
    <?php
        if ($still_member == true):?>
<!--            <div class="alert alert-success alert-dismissable fade show">-->
<!--                <button type="button" class="close" data-dismiss="alert">×</button>-->
<!--                <strong>Success!</strong> --><?php //echo "You choose user ".$invitedUserName," as administrator successfully"?>
<!--            </div>-->
        <?php
            $query_choose_ad = "UPDATE wu
                                SET wutype = 'SELECTED_ADMIN'
                                WHERE wid = '$workspaceID' and uid = '$invitedUserNameID';";
            $result_choose_ad = mysqli_query($conn, $query_choose_ad);
            if (!$result_choose_ad):
                echo '<script>alert("Error! Can\'t choose administrator!");</script>';
            endif;
            $url = "workspace_administrator.php?admin=$adminID&wname=$workspaceID";
            echo "<script type='text/javascript'>";
            echo "alert(\"You choose user $invitedUserName as an administrator successfully!\");";
    ...
    <?php
            ...
            <div class="alert alert-danger alert-dismissable fade show">
                <button type="button" class="close" data-dismiss="alert">×</button>
                <strong>Fail!</strong> <?php echo "You've already chosen user ".$invitedUserName?>
            </div>
        <?php
        endif;?>
    <?php
        ...
        //重定向浏览器
        header("Location: workspace_home.php?admin=$adminID");
        //确保重定向后，后续代码不会被执行
        exit;
        ?>
    <?php
...
<?php
else: //页面被第一次加载时，从workspace_home页面获得admin和wname的参数
    $adminID = $_GET['admin']; $admin = getUsername($adminID);
    $workspaceID = $_GET['wname']; $wname = getWorkspaceName($workspaceID);
    //$channelID = $_GET['cname']; $cname = getChannelName($workspaceID, $channelID);
    $not_deleted_removed = workspaceDeleted_memberRemoved($adminID, $workspaceID);
...
if ($conn)://若连接数据库成功
    $query_find_ad = "select uid, email
from wu natural join users
where wid = '$workspaceID' and uid in
                  (select uid
                  from wu
                  where wid = '$workspaceID' and wutype = 'MEMBER');";
    $result = mysqli_query($conn, $query_find_ad);
...
<?php endif;
    ...
    /************************************************************
     * 数据库查询wname中所有不在cname里人的姓名并赋给$username
     * 数据库查询wname中所有不在cname里人的邮箱并赋给$email
     * 数据库查询wname中所有不在cname里人的个数并赋给$user_num
        ...
        <div class="col-10">
            <div class="tab-content" id="nav-tabContent">
                <nav class="navbar navbar-expand-sm navbar-light bg-light justify-content-between">
                    <ul class="navbar-nav">
                        <li class="nav-item active">
                            <a class="navbar-brand" href="#">Choose Administrator</a>
                        </li>
                    </ul>
                </nav>
                <div class="card my-2">
                    <div class="card-body">
                        <h5 class="card-title"><?php echo getWorkspaceName($workspaceID) ?></h5>
                        <h6 class="card-subtitle mb-2 text-muted"><?php echo "You can only choose users who are members in ".getWorkspaceName($workspaceID)," as administrators."?></h6>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-outline-success"
                                                    onclick="javascript:window.location.href='<?php echo "workspace_administrator.php?admin=".$adminID,"&uname=".$username[$i],"&wname=".$workspaceID?>'">
                                                choose
                                            </button>
                                        </td>
                                    </tr>
