...
if ( !(isset( $_SESSION['username']) && isset($_GET['admin']) && $_SESSION['username'] == $_GET['admin']) ):    //用户尚未登录，必须重新登录
    ...
    $conn = custom_connect();
if (isset($_GET['cname'])): ////页面被第一次加载时，从channel_home页面获得admin和cname和wname的参数
    ...
    $deleted_removed = channelDeleted_memberRemoved($adminID, $workspaceID, $channelID);
    if (! $deleted_removed){
        $url = "channel_home.php?admin=$adminID&wname=$workspaceID";
        echo "<script type='text/javascript'>";
        echo "alert(\"You were removed from the channel $cname or this channel has been deleted!\");";
        echo " location.href='$url';";
        echo "</script>";
        exit;
    }
    $user_num = 0;$username = array(); $nickname = array();$email = array();$userType = array();
    $query = "select username, nickname, email, cutype
from users natural join cu
...
<!--                        <h6 class="card-subtitle mb-2 text-muted">--><?php //echo "You can only invite users in the same workspace to ".$cname?><!--</h6>-->
