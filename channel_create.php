...
$wid = $workspaceID; $deleted_removed_workspace = workspaceDeleted_memberRemoved($adminID, $workspaceID);
/*需要传入的变量*/
...
if (isset($_POST['create_c'])):
    ...
    <?php else:
        ...
        $stmt = $conn->prepare("insert into Channels (wid, cname, ctype, ccreatetime) values (?,?,?,?);");//向数据库插入表单传来的值的sql
        $stmt->bindParam(1, $wid, PDO::PARAM_INT, 11);
        $stmt->bindParam(2, $cname, PDO::PARAM_STR, 32);
        $stmt->bindParam(3, $channeltype, PDO::PARAM_STR, 20);
        $stmt->bindParam(4, $now, PDO::PARAM_STR, 20);
        $result1 = $stmt->execute();


        $stmt = $conn->prepare("select cid from Channels where cname = ? and ctype = ? and wid = ? and ccreatetime = ?;");
        $stmt->bindParam(1, $cname, PDO::PARAM_STR, 32);
        $stmt->bindParam(2, $channeltype, PDO::PARAM_STR, 20);
        $stmt->bindParam(3, $wid, PDO::PARAM_INT, 11);
        $stmt->bindParam(4, $now, PDO::PARAM_STR, 20);
        $result2 = $stmt->execute();
        $newcid = $stmt->fetchAll(PDO::FETCH_ASSOC)[0]['cid'];

        $stmt = $conn->prepare("insert into CU (wid, cid, uid, cutype) values (?,?,?,'CREATOR');");//向数据库插入表单传来的值的sql
        $stmt->bindParam(1, $wid, PDO::PARAM_INT,11);
        $stmt->bindParam(2, $newcid, PDO::PARAM_INT,11);
        ...
        if ($result1 != False and $result2 != False and $result3 != False):?>
            ...
            <meta http-equiv="refresh" content="2; url=<?php echo "channel_create.php?admin=$adminID&wname=$workspaceID" ?>">
        <?php else: ?>
            <div class="alert alert-danger alert-dismissable fade show">
                <button type="button" class="close" data-dismiss="alert">×</button>
                <strong><?php echo "Channel already exist!" ?></strong>
            </div>
        <?php endif; ?>
    <?php endif; ?>
<?php endif; ?>
                            ...
                            <div class="col-xl-6 col-lg-10 col-md-9">
                                <div class="card text-center my-5">
                                    ...
                                    <div class="list-group list-group-flush">
                                        <form accept-charset="UTF-8" role="form" action="<?php echo "channel_create.php?admin=".$adminID,"&wname=".$workspaceID?>" method="post">
                                            <fieldset>
                                                <div class="list-group-item">
                                                    <input class="form-control" placeholder="Channel name" name="channel_name"
                                                           type="text">
                                                </div>
                                                <div class="list-group-item text-left">
                                                    ...
                                                    <div class="radio">
                                                        <label>
                                                            <input type="radio" name="channeltype" id="PUBLIC" value="PUBLIC"
                                                                   checked> PUBLIC
                                                        </label>
                                                    </div>
                                                    <div class="radio">
                                                        <label>
                                                            <input type="radio" name="channeltype" id="DIRECT" value="DIRECT">
                                                            DIRECT
                                                        </label>
                                                    </div>
                                                    <div class="radio">
                                                        <label>
                                                            <input type="radio" name="channeltype" id="PRIVATE" value="PRIVATE">
                                                            PRIVATE
                                                        </label>
                                                    </div>
                                                </div>
